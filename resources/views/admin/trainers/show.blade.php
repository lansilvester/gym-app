...
<div class="max-w-3xl space-y-4">
    <div class="flex items-center gap-2">
        <a href="{{ route('admin.trainers.index') }}" class="text-gray-500 hover:text-gray-700">&larr; Back</a>
        <h1 class="text-2xl font-bold text-gray-800">Trainer: {{ $trainer->user->name }}</h1>
    </div>

    <div class="bg-white rounded-lg shadow p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-500">Trainer Code</label>
            ...
            <div>
                ...
                <p class="text-sm text-gray-800">{{ $trainer->is_available ? 'Yes' : 'No' }}</p>
            </div>
        </div>
        @if($trainer->bio)
        <div>
