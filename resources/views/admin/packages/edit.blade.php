...
@section('content')
<div class="max-w-2xl">
    <h1 class="text-2xl font-bold text-gray-800 mb-4">Edit Package: {{ $package->name }}</h1>
    ...
    <form method="POST" action="{{ route('admin.packages.update', $package) }}" class="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="max_checkin_per_week" value="{{ old('max_checkin_per_week', $package->max_checkin_per_week) }}" min="0" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Leave empty for unlimited">
            </div>
            <div>
        ...
        <div class="flex gap-2 pt-4">
            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg text-sm hover:bg-blue-700">Update Package</button>
