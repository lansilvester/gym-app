@extends('layouts.app')
@section('title', 'Paket Keanggotaan')

@section('content')
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-800">Paket Keanggotaan</h1>
        <a href="{{ route('admin.packages.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm">+ Tambah Paket</a>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-medium text-gray-600">Durasi</th>
                    <th class="px-4 py-3 text-left font-medium text-gray-600">Harga</th>
                    <th class="px-4 py-3 text-left font-medium text-gray-600">Maks Check-in</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3">{{ $package->duration_days }} hari</td>
                    <td class="px-4 py-3">Rp {{ number_format($package->price, 0, ',', '.') }}</td>
                    <td class="px-4 py-3">{{ $package->max_checkin_per_week ?? 'Tidak terbatas' }}</td>
                    <td class="px-4 py-3">
                        @if($package->includes_personal_training)
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">Ya</span>
                        @else
                    ...
                    <td class="px-4 py-3">
                        <a href="{{ route('admin.packages.edit', $package) }}" class="text-yellow-600 hover:underline text-xs">Ubah</a>
                        <form method="POST" action="{{ route('admin.packages.destroy', $package) }}" class="inline ml-2" onsubmit="return confirm('Apakah Anda yakin?')">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr><td colspan="8" class="px-4 py-8 text-center text-gray-500">Tidak ada paket ditemukan</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
