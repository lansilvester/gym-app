...
@section('content')
<div class="max-w-2xl">
    <h1 class="text-2xl font-bold text-gray-800 mb-4">Create Invoice</h1>
    ...
    <form method="POST" action="{{ route('admin.invoices.store') }}" class="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="description" value="{{ old('description') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="e.g. Membership Renewal, Personal Training Session" required>
        </div>
        ...
        <h3 class="font-semibold text-gray-700 border-b pb-2">Invoice Items</h3>
        <div id="items-container" class="space-y-3">
            <div class="item-row grid grid-cols-12 gap-2 items-end">
                ...
                <div class="col-span-3">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Unit Price (Rp)</label>
        ...
        <button type="button" onclick="addItem()" class="text-blue-600 hover:underline text-sm">+ Add Item</button>

        <div class="flex gap-2 pt-4">
            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg text-sm hover:bg-blue-700">Create Invoice</button>
            <a href="{{ route('admin.invoices.index') }}" class="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg text-sm hover:bg-gray-300">Cancel</a>
        </div>
    </form>
</div>
