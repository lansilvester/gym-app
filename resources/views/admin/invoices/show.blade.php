...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-800">Faktur {{ $invoice->invoice_number }}</h1>
        <div class="flex gap-2">
            <a href="{{ route('admin.invoices.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm hover:bg-gray-300">Kembali</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-white rounded-lg shadow p-6">
            <h3 class="font-semibold text-gray-800 mb-4">Informasi Faktur</h3>
            <div class="space-y-3 text-sm">
                <div class="flex justify-between"><span class="text-gray-500">No. Faktur</span><span class="font-mono font-medium">{{ $invoice->invoice_number }}</span></div>
                <div class="flex justify-between"><span class="text-gray-500">Anggota</span><span>{{ $invoice->member->user->name ?? '-' }}</span></div>
                <div class="flex justify-between"><span class="text-gray-500">Dibuat</span><span>{{ $invoice->created_at->format('d M Y') }}</span></div>
                <div class="flex justify-between"><span class="text-gray-500">Jatuh Tempo</span><span>{{ $invoice->due_date?->format('d M Y') ?? '-' }}</span></div>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="font-semibold text-gray-800 mb-3">Item Faktur</h3>
                <table class="w-full text-sm">
                    <thead class="border-b border-gray-200">
                        <tr>
                            <th class="text-left py-2 text-gray-600">Deskripsi</th>
                            <th class="text-right py-2 text-gray-600">Jumlah</th>
                            <th class="text-right py-2 text-gray-600">Harga Satuan</th>
                    ...
                    <tfoot class="border-t border-gray-200">
                        ...
                        <tr>
                            <td colspan="3" class="py-1 text-right text-gray-500">Dibayar:</td>
                            <td class="py-1 text-right text-green-600">Rp {{ number_format($invoice->amount_paid, 0, ',', '.') }}</td>
                        </tr>
                        <tr>
                            <td colspan="3" class="py-1 text-right text-gray-500">Sisa:</td>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @empty
                        <tr><td colspan="4" class="py-4 text-center text-gray-500">Belum ada pembayaran tercatat</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
