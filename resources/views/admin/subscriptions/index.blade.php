...
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-800">Langganan</h1>
        <a href="{{ route('admin.subscriptions.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm">+ Tambah Langganan</a>
    </div>
    ...
    <form method="GET" class="flex gap-2">
        <div class="relative flex-1">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari berdasarkan nama anggota atau paket..." class="w-full border border-gray-300 rounded-lg px-3 py-2 {{ request('search') ? 'pr-9' : '' }} text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            @if(request('search'))
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-medium text-gray-600">Paket</th>
                    <th class="px-4 py-3 text-left font-medium text-gray-600">Tanggal Mulai</th>
                    <th class="px-4 py-3 text-left font-medium text-gray-600">Tanggal Akhir</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3">
                        ...
                        @elseif($subscription->status == 'expired')
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">Kadaluarsa</span>
                        @else
                ...
                @empty
                <tr><td colspan="6" class="px-4 py-8 text-center text-gray-500">Tidak ada langganan ditemukan</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
